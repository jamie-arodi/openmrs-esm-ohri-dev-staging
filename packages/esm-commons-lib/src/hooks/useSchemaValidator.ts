...
import { OHRIFormField, OHRIFormSchema, ConceptTrue, ConceptFalse } from '@openmrs/openmrs-form-engine-lib';

export function useSchemaValidator(schema, doValidate) {
  ...
  const { dataTypeToRenderingMap, conceptDataTypes } = useConfig() as ConfigObject;
